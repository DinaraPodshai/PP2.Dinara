...
clock = pygame.time.Clock() #контролировать количество кадров 1
FPS = 6 #скорость

def draw_grid():
  for i in range(0, WINDOW_WIDTH, BLOCK_SIZE):
    for j in range(0, WINDOW_HEIGHT, BLOCK_SIZE):
      pygame.draw.rect(screen, GREY, (i, j, BLOCK_SIZE, BLOCK_SIZE), 1)
...
class Food:
  ...
  def my_round(self, value, base=BLOCK_SIZE):
    return base * round(value / base) #Округляет координаты до ближайшего блока сетки.
  
  def generate_random_pos(self): #еду
  ...
  def respawn(self): #перегенерирует
    self.generate_random_pos()
